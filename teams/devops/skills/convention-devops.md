---
name: convention-devops
description: DevOps 인프라 컨벤션 — IaC, CI/CD, 배포 전략, 모니터링, SLA
team: devops
trigger: "컨벤션|convention|인프라 규칙|DevOps 규칙"
---
...
GitHub Actions 기반:
```
코드 푸시 → 빌드 → 테스트 → 스테이징 배포 → 승인 → 프로덕션 배포
```
...
## 모니터링
- 플랫폼: Datadog (APM, 로그, 메트릭, 트레이싱)
- 알림: Datadog 알럿 → PagerDuty (온콜) → Slack 채널
