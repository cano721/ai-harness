...
assert_eq "4" "$(jq -r 'select(.kind=="error") | .n' "$CLAUDE_EVENT")" "Claude tool error counts is_error results"
assert_eq "1" "$(jq -r 'select(.kind=="guard_block") | .n' "$CLAUDE_EVENT")" "Claude guard block counts edit-tool is_error results only, not Bash/Read output"
assert_eq "1" "$(jq -r 'select(.kind=="permission_deny") | .n' "$CLAUDE_EVENT")" "Claude permission denial requires is_error and leading phrase (ignores clarify, source text, git log)"
...
assert_eq "2" "$(jq -r 'select(.kind=="error") | .n' "$CODEX_EVENT")" "Codex tool error counts bridge envelope and non-zero exit, not source text"
