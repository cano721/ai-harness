...
2. planning 선택 시: runtime-aware 글로벌 설치
   - 사용 도구를 사용자에게 직접 확인한다. 환경 변수로 자동 감지하지 않는다.
     ```
     "어떤 AI 도구를 사용 중인가요?
      ...
      선택 (기본: 1):"
     ```
   - 선택에 따라 `--runtime` 값을 결정한다:
     - `1` 또는 엔터 → `--runtime claude`
     - `2`            → `--runtime codex`
   - runtime에 따라 설치 소스를 고른다.
     - Claude Code → `teams/planning/bundle-claude/`
     - Codex       → `teams/planning/bundle-codex/`
   - 먼저 아래 inspect 명령으로 설치 대상을 미리 보여준다:
     ```bash
     node scripts/install-planner-bundle.mjs inspect --runtime claude
     ```
     ```
     "planning 팀 글로벌 세팅:
       runtime: Claude Code
   ...
   - 확인 후 아래 install 명령을 실행한다:
     ...
     ```
     - Codex를 선택했다면 위 명령의 `claude`를 `codex`로 바꿔 실행한다.
     - Codex: `bundle-codex/`의 `AGENTS.md`, `agents/*.toml`, `skills/`, `planner-templates/` 설치
     - Claude:
       - `bundle-claude/CLAUDE.md` → `~/.claude/CLAUDE.md`
       - `bundle-claude/agents/*.md` → `~/.claude/agents/*.md`
       - `bundle-claude/skills/` → `~/.claude/plugins/marketplaces/ai-harness/skills/`
       - `bundle-claude/planner-templates/` → `~/.claude/planner-templates/`
       - 설치 후 Claude Code 네이티브 subagent 구성이 더 필요하면 `refresh-planning-subagents` skill을 한 번 실행한다.
   - 설치 후 readiness를 비차단으로 점검한다:
     - `~/.claude/credentials.md`에 Atlassian 섹션 존재 여부
     - jira, jira-checklist skill 존재 여부
     - policy template 설치 여부
     - Claude를 선택한 경우 `refresh-planning-subagents` skill 경로 존재 여부
...
3.5 [v2] 프로젝트 맞춤 에이전트 생성 (Harness — AI 활용 최적화)
   ...
   - frontmatter는 Claude Code 공식 subagent 형식에 맞춰 `name`, `description`을 필수로 두고, 필요 시 `tools`, `model` 같은 지원 필드만 사용한다
...
3.7 [v2] Task Workflow 설치
   - 팀 구성이 아닌 **작업 유형** 기반으로 워크플로우를 설치한다
   - `templates/task-workflows/` 에서 팀에 해당하는 워크플로우를 `.ai-harness/task-workflows/`로 복사
   - 각 워크플로우의 `{project}` 플레이스홀더를 실제 프로젝트명으로 치환
   - 설치할 워크플로우 목록을 사용자에게 보여주고 확인:
     ```
     "Task Workflow 설치:
     ...
     ```
   - CLAUDE.md에 task-workflow 참조 규칙을 주입:
     - "기능 구현 요청 시 .ai-harness/task-workflows/implement-feature.yaml의 phases를 따르라"
     - "각 phase의 agent를 Agent 도구로 서브에이전트 호출하여 실행하라"
     - "review phase는 반드시 implement과 다른 서브에이전트로 실행하라 (enforce_separation)"
