...
12. OMC 설치 권장 (선택)
    - Bash로 `claude plugin list` 실행하여 OMC 설치 여부 확인
    - 이미 설치되어 있으면 스킵
    - 미설치 시 안내:
      ```
      "OMC(oh-my-claudecode)가 설치되어 있지 않습니다.
       autopilot, ralph 등 AI 활용 효율을 높이는 오케스트레이션 도구입니다.
       설치하시겠습니까? (Y/n)"
      ```
    - Y 선택 시: Bash로 `claude plugin install oh-my-claudecode@omc` 실행
    - 설치 완료 후: "OMC 설치 완료. 새 세션에서 `omc setup`을 실행하세요." 안내
</Steps>
