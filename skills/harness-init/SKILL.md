...
<Steps>
1. 글로벌 보안 세팅 (자동)
   - ~/.claude/settings.json에 보안 Hook 4개(block-dangerous, secret-scanner, check-architecture, audit-logger)가 등록되어 있는지 확인
   - 없으면 자동 등록, 있으면 스킵
   - 사용자에게 질문하지 않음 (보안 Hook은 필수)
   - 등록 결과를 보고: "글로벌 보안 Hook N개 등록 / M개 이미 존재"

2. 환경 감지
   - Bash로 `node scripts/check-environment.mjs` 실행
   - Node.js, Git, Claude Code 버전 확인

3. 팀 선택 (프로젝트 분석보다 먼저 — 팀에 따라 분석 범위가 달라짐)
...
4. 프로젝트 분석 (선택된 팀 기반으로 집중 분석)
   - 선택된 팀에 따라 분석 대상이 달라짐:
     - **backend**: 패키지 구조, Controller/Service/Repository 패턴, DTO 네이밍, API 경로, DB 설정
     - **frontend**: 컴포넌트 구조, 상태 관리, API 호출 패턴, 라우팅, 스타일링
     - **devops**: CI/CD 설정, Dockerfile, k8s 매니페스트, 환경 변수
     - **qa**: 테스트 프레임워크, 디렉토리 구조, 커버리지 설정
     - **design**: 디자인 시스템, 토큰, 컴포넌트 라이브러리
     - **planning**: 기존 문서, 이슈 추적기 연동
   - 주요 소스 파일을 Read로 읽어서 패턴 파악
   - 분석 결과를 사용자에게 보고하고 확인

5. 프로젝트 정보 수집 (도메인은 프로젝트에 속함, 팀에 속하지 않음)
...
11. 컨텍스트 맵 생성
    - templates/context-map.md 템플릿을 Read
    - 프로젝트 분석 결과로 플레이스홀더를 채움:
      - {project.name}, {project.domain}, {project.tech_stack}, {project.base_package}
      - 핵심 진입점: 실제 디렉토리 구조 기반
      - 주요 엔티티: 분석된 Entity 목록 + 관계
      - 공통 패턴: 컨벤션에서 감지된 응답 클래스, 예외, DTO 패턴
      - 자주 수정하는 파일: `git log --format='' --name-only | sort | uniq -c | sort -rn | head -10`
    - .ai-harness/context-map.md 에 Write로 저장
    - "이 파일은 에이전트를 위한 지도입니다. 전체 매뉴얼 대신 핵심만 담습니다." 안내
