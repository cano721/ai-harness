...
- 노트를 읽지 못하면 플러그인 루트의 `CHANGELOG.md`에서 해당 버전 절을 읽는다: `$ROOT/scripts/changelog-section.sh <version>`. 설치본에 함께 배포되므로 네트워크 없이도 동작한다. 다만 **설치된 버전까지의 이력만** 담고 있으므로, 아직 설치하지 않은 최신 버전의 절은 없을 수 있다.
- 그마저 없으면 링크만 제시한다. 커밋 로그나 버전 번호로 변경점을 추측해 쓰지 않는다.
...
Claude Code에서 적용했다면 다음부터 수동 업데이트가 필요 없도록 마켓플레이스 자동 업데이트를 켜는 법을 한 줄로 안내한다: `/plugin` → Marketplaces → ai-harness → Enable auto-update. 서드파티 마켓플레이스는 기본값이 꺼져 있고 사용자만 켤 수 있으므로, 설정 파일을 직접 고쳐 켜지 않는다. Codex에는 이 안내를 하지 않는다.
