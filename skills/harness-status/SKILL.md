...
4.5 [v2] Harness 상태:
   - .ai-harness/agents/ 디렉토리 확인 → 생성된 에이전트 목록 표시
   - .claude/agents/와 동기화 여부 확인
   - .ai-harness/workflow.md 존재 여부 → 워크플로우 패턴 표시
   - .ai-harness/teams/{team}/skills/develop-*.md 존재 여부 → 전문 스킬 표시
   - config.yaml의 _schema_version 확인 (v1이면 업그레이드 권장)
...
5. 출력 형식:
   ...
   [Guide] 컨벤션
     ✅ 팀: backend
     ✅ Hook: sql-review, api-compat, entity-review, coverage-check

   [Harness] AI 활용 최적화
     ✅ 에이전트: myapp-developer, myapp-reviewer, myapp-architect
     ✅ 스킬: develop-backend, review-backend
     ✅ 워크플로우: Pipeline
     ⚠ .claude/agents/myapp-reviewer.md 동기화 필요
