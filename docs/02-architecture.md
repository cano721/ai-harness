...
AI Harness는 에이전트를 직접 실행하는 제품이라기보다, 프로젝트별 규칙과 런타임 연결 상태를 정리해 주는 control plane이다. 이 문서는 그 control plane이 Guard, Guide, Gear 세 축 위에서 어떻게 구성되는지 설명한다.
...
현재 모노레포는 control plane과 execution adapter를 분리해서 관리한다.

```text
packages/
├── ui/                       # 운영 UI
├── server/                   # API, 서비스, 레지스트리 조합
├── shared/                   # 공용 타입/상수
├── db/                       # 스키마/마이그레이션
├── adapter-utils/            # 어댑터 공통 인터페이스 + 프로세스 실행기
├── adapter-claude-local/     # Claude 로컬 런타임 어댑터
├── adapter-codex-local/      # Codex 로컬 런타임 어댑터
└── adapter-cursor-local/     # Cursor 로컬 런타임 어댑터
```

이렇게 나누면 서버는 특정 런타임 구현을 직접 품지 않고, 어떤 어댑터를 레지스트리에 조립할지만 책임진다.

## Control Plane 관점

- **Execution plane은 외부 런타임**: Claude Code, Codex, Cursor 같은 실제 에이전트 런타임이 코드를 작성하고 도구를 호출한다.
- **AI Harness는 운영 계층**: 어떤 규칙을 적용할지, 어떤 스킬을 로드할지, 어떤 팀 자산을 프로젝트에 주입할지를 결정한다.
- **프로젝트 로컬 상태가 핵심**: `.ai-harness/` 안의 설정과 지도 파일이 일상 작업의 사실상 계약서 역할을 한다.

이 구분을 유지하면 "하네스가 너무 많은 실행 책임을 가져가는 것"을 방지하고, 런타임 종속성도 낮출 수 있다.
